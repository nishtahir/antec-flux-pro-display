...
fn encode_temperature(temp: f32) -> String {
    let integer_part = (temp / 10.0) as u8;
    let tenths_part = (temp % 10.0) as u8;
    let hundredths_part = ((temp * 10.0) % 10.0) as u8;
    format!(
        "{:02x}{:02x}{:02x}",
        integer_part, tenths_part, hundredths_part
    )
}

fn generate_payload(cpu_temp: f32, gpu_temp: f32) -> Vec<u8> {
    let cpu_encoded = encode_temperature(cpu_temp);
    let gpu_encoded = encode_temperature(gpu_temp);

    let combined_str = format!("{}{}", cpu_encoded, gpu_encoded);
    let combined_bytes = hex::decode(&combined_str).unwrap();

    let checksum = combined_bytes.iter().sum::<u8>().wrapping_add(7);

    // Generate the final payload
    let payload_hex = format!("55aa010106{}{}{:02x}", cpu_encoded, gpu_encoded, checksum);
